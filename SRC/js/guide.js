...
function fnGuideEditCheck() {
  ...
  for (i = 0; i < cnt; i++) {
    ...
    if (!fnYMDCheck(msg + "開始日付には正しい日付", tmp)) {
      return;
    }
    if (
      !fnYMDCheck(
        msg + "開始日付には正しい日付",
        getEditObject(i, "guideStartDT")
      )
    ) {
      return;
    }
    ...
    if (
      !fnYMDCheck(msg + "受付日には正しい日付", getEditObject(i, "acceptDT"))
    ) {
      return;
    }
    if (isLength(100, msg + "受付", getEditObject(i, "accept"))) {
      return;
    }
  }
...
function fnGuideMove(muki, articleNo) {
  if (muki == "toRight") {
    ...
    tags += "<td>";
    tags +=
      '<input type="button" value="&lt;" onClick="fnGuideMove( \'toLeft\', ' +
      articleNo +
      ' );" />';
    tags +=
    ...
    // 登録ボタンの作成
    tags +=
      '<td><input type="button" value="&gt;" onClick="fnGuideMove( \'toRight\', \'' +
      articleNo +
      "' );\" /></td>";
    tags += "</tr>";
